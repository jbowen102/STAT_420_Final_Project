...
```{r, eBird_filters, results = "hide", message = FALSE, warning = FALSE}
...
Here are the observation locations plotted on a map of the state:  

![](./output/ebd_data_raven_SC_2024.png)
...
add_dist_to_coast = function(ebd_df) {
    ...
    for (x in 1:nrow(ebd_df)) {
        ebd_df[x, "dist_to_coast_mi"] = -round(0.62137 * get_dist_from_coast(ebd_df[[x, "latitude"]], ebd_df[[x, "longitude"]]), 3)
    }
    ebd_df
}
```
...
head(ebd_df_with_elev_and_weather_and_dist_to_coast[c("global_unique_identifier",
                                                      "observation_count",
                                                      "latitude",
                                                      "longitude",
                                                      "observation_date",
...
Here is what the total dataset looks like, with predictors in light blue and the response variable in green:  
